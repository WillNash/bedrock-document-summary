"""
Tests for tools/consistency_evaluator.py — pathway 1 (variability measurement).

Validates the similarity computation and aggregation logic using mocked and
synthetic responses. No real Bedrock calls or model downloads are made in the
orchestration tests; the embedding and BERTScore tests download small models
(all-MiniLM-L6-v2 ~91 MB, distilbert-base-uncased ~260 MB unverified).
...
# Single module-level guard: skip the entire file if any dep is absent, rather
# than using multiple importorskip calls which can cause partial-skip issues.
_MISSING_DEPS = [
    m
    for m in ("numpy", "sentence_transformers", "bert_score", "sklearn")
...
# Uses the established repo pattern (importlib.util.spec_from_file_location).
# Because consistency_evaluator.py uses lazy imports for sentence_transformers
# and bert_score, exec_module() succeeds even when those packages are absent.
...
# Register under the bare module name so mock.patch.object targets resolve.
